<template>
    <scroll ref="tryingWrapper" :data="data" :pullup="state.pullup" @scrollEnd="scrollEnd">
      <ul>
            <li v-for="item in data" class="try-item" :key="item.id">
                ...
                <div v-if="item.list.length > 0" class="applyPers" @click.prevent="toApplyList(item)">
                    <apply-persons :list="item.list" :showmore="state.showmore" :limit="state.limit" class="persons"></apply-persons>
...
export default {
  ...
  setup (props, { emit }) {
    ...
    function toApplyList (tryitem) {
      emit('to-apply-list', tryitem.id)
    }
    function scrollEnd () {
      emit('scroll-end')
    }
